...
parser.add_argument("--model", type=str, default="../ckpt/VDSR/VDSR_x4.pth", help="model path")
parser.add_argument("--model_sam", type=str, default="../ckpt/VDSR/VDSR_SAM_x4.pth", help="model_sam path")
...
def valid_sam(test_loader, model):
    ...
    with torch.no_grad():
        ...
        print("===> VDSR_SAM Avg. PSNR: {:.8f} dB, Avg. SSIM: {:.8f}".format(psnr_epoch/(iteration+1), ssim_epoch/(iteration+1)))
...
def valid(test_loader, model):
    ...
    with torch.no_grad():
        ...
        print("===> VDSR Avg. PSNR: {:.8f} dB, Avg. SSIM: {:.8f}".format(psnr_epoch/(iteration+1), ssim_epoch/(iteration+1)))
