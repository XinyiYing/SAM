...
parser.add_argument("--model_sam", type=str, default="../best_model/VDSR/VDSR_SAM_x4.pth", help="model path")
opt = parser.parse_args()
weights_sam = torch.load(opt.model_sam)['model'].state_dict()
model = Net_SAM(n_intervals=[6,12])
model1 = model.state_dict
#--------------------------------------------------------
a = weights_sam['sam_layer.0.bottleneck.weight' ]
b = torch.zeros(64,129,1,1)
b[:,0:128,:,:] = a
weights_sam['sam_layer.0.bottleneck.weight' ] = b

a = weights_sam['sam_layer.1.bottleneck.weight' ]
b = torch.zeros(64,129,1,1)
b[:,0:128,:,:] = a
weights_sam['sam_layer.1.bottleneck.weight' ] = b
#----------------------------------------------------------
import vdsr_sam
model_sam1 = vdsr_sam.Net_SAM(n_intervals=[6, 12])
model_sam1.load_state_dict(weights_sam)
model_out_path = "../best_model/VDSR/VDSR_SAM_x41.pth"
state = {"epoch": torch.load(opt.model_sam)['epoch'], "model": model_sam1}
